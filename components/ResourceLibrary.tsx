...
import { Play, Book, FileText, Code, Monitor } from 'lucide-react';
...
export const ResourceLibrary: React.FC = () => {
  ...
  const filteredResources = activeTab === 'All' 
    ? RESOURCES 
    : RESOURCES.filter(r => {
        if (activeTab === 'Paper') return r.type.includes('Paper') || r.type === 'Whitepaper';
        if (activeTab === 'Book') return r.type.includes('Book');
        return r.type === activeTab;
    });

  const getIcon = (type: string) => {
    if (type.includes('YouTube') || type.includes('Video')) return <Play size={20} />;
    if (type.includes('Book')) return <Book size={20} />;
    if (type.includes('Course')) return <Monitor size={20} />;
    if (type.includes('Paper') || type.includes('Article')) return <FileText size={20} />;
    return <Code size={20} />;
  };
    ...
    <section id="library" className="py-20 bg-black/20">
      <div className="container mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {filteredResources.slice(0, 16).map((res, idx) => (
            <Card key={idx} className="group hover:border-aether-secondary/30 transition-all">
        ...
        {filteredResources.length > 16 && (
            <div className="mt-12 text-center">
                <button className="px-6 py-3 rounded-lg border border-white/10 hover:bg-white/5 text-gray-300 text-sm font-medium transition-colors">
